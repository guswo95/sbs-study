...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField] Color ColorBoosHp;
    ...
    private int GetRank() // 플레이어가 사망했을때 현재 점수가 랭크로부터 몇등인지 가져오는 함수
    {
    ...
    {
        if (objGameoverMenu.activeSelf == true) // 오브젝트가 active true 되어 있었다면
        {
    ...
    {
        //Json 제이슨 key, value 구성 , 기본구조 string
        string rankValue = PlayerPrefs.GetString(keyRankData, string.Empty); //데이터가 없을시 ""; 이렇게 표시
    ...
    {
        //랭크에 들었다면 이름과 점수를 ranklist에 저장해주고 11등은 삭제
        if(rank != -1)
        {
    ...
    public void GameOver()//플레이어가 상망했을때 동작
    {
        //랭크에 들었다면 이름과 점수를 기록할수있는 메뉴
        rank = GetRank(); // 랭크에 들지 않았다면 -1
        if(rank !=  -1)
        {
            textGameover.text = $"{rank}등 달성!";
            iFGameover.gameObject.SetActive(true);
            //iFGameover.onFocusSelectAll = true;
        }
