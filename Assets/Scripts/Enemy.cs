...
public class Enemy : MonoBehaviour
{
    ...
    int curPatternShootCount = 0; // 패턴을 몇번 사용했는지
    float patternTimer;
    bool patternChange = false; // 패턴을 바꾸어야 하는 상황인지
    [SerializeField] float patternChangeTime = 1f; //패턴을 바꿀대 딜레이 되는 시간
    ...
    private void shootGatling()
    {
        ...
        float angle = Quaternion.FromToRotation(Vector3.up, playerPos - transform.position).eulerAngles.z; // 알아두면 좋음 
    ...
    {
        bool isNameBossHit = anim.GetCurrentAnimatorStateInfo(0).IsName("BoosHit");//이름이 보스 히트라면 true 아니면 false
